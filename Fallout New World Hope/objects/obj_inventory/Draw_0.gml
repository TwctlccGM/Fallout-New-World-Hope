...
{ 
	...
	draw_sprite_stretched(spr_textbox_ph, 0, _xx, _yy, 160, 180);
	draw_set_font(fnt_fixedsys);
	draw_text(_xx + 5, _yy + 5, "ITEMS");
	for(var _pos = 0; _pos < 5; _pos++)
    {
		if !(global.item_array[_pos, C_ITEM_TYPE] == ITEM_NONE)
        {
			draw_sprite(global.item_array[_pos, C_ITEM_SPRITE], 0, _xx + 20, _yy + 40);
			draw_text(_xx + 35, _yy + 30, string(global.item_array[_pos, C_ITEM_AMOUNT]));
        }
		_yy += 25;
	}
	...
	draw_sprite_stretched(spr_textbox_ph, 0, _xx, _yy, 160, 180);
	draw_set_font(fnt_fixedsys);
	draw_text(_xx + 5, _yy + 5, "PARTY");
	for(var _pos = 0; _pos < array_length(global.party); _pos++)
    {
		draw_sprite(global.party[_pos].sprites.idle, 0, _xx + 30, _yy + 45);
		draw_text(_xx + 50, _yy + 30, "HP: " + string(global.party[_pos].hp) + "/" + string(global.party[_pos].hp_max));
		draw_healthbar(_xx + 80, _yy + 45, _xx + 140, _yy + 46, global.party[_pos].hp, c_gray, c_red, c_green, 0, true, false);
		_yy += 50;
	}
